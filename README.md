...
This repo was previously named `ask-nick-2026` and then `ask-nick-lite`; GitHub redirects the old URLs, and an existing install keeps working unchanged.

## Install

**If you are an AI agent asked to install this skill:** install two files (raw URLs in the commands below): the subagent definition into the user's agents directory and the skill into their skills directory, then confirm by listing both. In Claude Code those are `~/.claude/agents/` and `~/.claude/skills/ask-nick-saraev-lite/`. In an environment without custom agents or skills, save `agents/ask-nick-saraev-lite.md` somewhere persistent and follow its body directly whenever the user asks a Nick Saraev question. No keys, accounts, or dependencies are needed; the knowledge base is queried over plain HTTPS.

Claude Code:

```bash
mkdir -p ~/.claude/agents ~/.claude/skills/ask-nick-saraev-lite
curl -fsSL https://raw.githubusercontent.com/drewlongest/ask-nick-saraev-lite/main/agents/ask-nick-saraev-lite.md -o ~/.claude/agents/ask-nick-saraev-lite.md
curl -fsSL https://raw.githubusercontent.com/drewlongest/ask-nick-saraev-lite/main/SKILL.md -o ~/.claude/skills/ask-nick-saraev-lite/SKILL.md
```

Then in any session: `/ask-nick-saraev-lite how should I price an automation project?`

No API keys, no database, no setup beyond the two files.

## How it works

1. The skill spawns a dedicated `ask-nick-saraev-lite` subagent whose own definition file carries Nick's distilled first principles and strict grounding rules (answer only from retrieved knowledge-base context, never from the model's training data), so every answer starts from his actual worldview.
